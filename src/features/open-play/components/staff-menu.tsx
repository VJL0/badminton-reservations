    ...
    <Menu.Root>
      ...
      >
        <span className="hidden flex-col items-end gap-0.5 lg:flex">
          ...
          <span className="font-mono text-caption text-ink-2 uppercase tracking-caps">{role.toLowerCase()}</span>
        </span>
        <span aria-hidden className="flex size-9 items-center justify-center rounded-full bg-ink font-bold text-sm text-white">
          {initial}
        </span>
        <svg
          aria-hidden
        ...
        <Menu.Positioner align="end" sideOffset={8} className="z-50">
          <Menu.Popup className="min-w-48 rounded-xl border bg-white p-1.5 text-ink shadow-lg outline-hidden">
            <div className="px-3 pt-1 pb-1.5 lg:hidden">
              ...
              <p className="font-mono text-caption text-ink-2 uppercase tracking-caps">{role.toLowerCase()}</p>
            </div>
