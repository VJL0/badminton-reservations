    ...
    <section aria-label="Waiting queue" className="flex flex-col gap-4 lg:gap-6">
      <div className="flex flex-wrap items-end justify-between gap-2">
        ...
        <p className="font-mono text-xs uppercase tracking-[0.12em] text-ink-2">
          First in, first on · estimates assume full-length games
        </p>
      </div>
            ...
            {Array.from({ length: QUEUE_ROW }, (_, k) => {
              ...
              return (
                ...
                >
                  ...
                  {q?.court_number && (
                    <span className="shrink-0 rounded-full bg-ink/10 px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.08em]">Ct {q.court_number}</span>
                  )}
                  {q && me.role && !isMe && (
