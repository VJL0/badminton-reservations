...
import { authorizeStaffForm, revokeStaff } from "@/features/open-play/actions/staff";
...
const authorize = settleForm(authorizeStaffForm);
...
export function AuthorizeStaffForm() {
  const [state, action, pending] = useActionState(authorize, null);

  return (
    <form action={action}>
      <FieldGroup>
        <Field>
          <FieldLabel htmlFor="staff-email">Google account email</FieldLabel>
          <Input
            id="staff-email"
            name="email"
            type="email"
            required
            autoComplete="off"
            placeholder="name@temple.edu"
            defaultValue={String(state?.values?.email ?? "")}
          />
          <p className="text-xs text-muted-foreground">
            Nothing is sent. They open the sign-in page and choose Continue with Google; Gmail and school or work accounts both work.
          </p>
        </Field>
        <Field>
          ...
          <select
            id="staff-role"
            name="role"
            defaultValue={String(state?.values?.role ?? "OPERATOR")}
            className="h-9 rounded-lg border border-input bg-transparent px-2.5 text-sm"
          >
            <option value="OPERATOR">Officer: runs the board</option>
            <option value="ADMIN">Admin: also sessions, settings and staff</option>
          </select>
        </Field>
        <FormFooter state={state} pending={pending} label="Authorize" />
      </FieldGroup>
    </form>
  );
}

/** Take someone's staff access away. Effective on their next tap. */
      ...
      <ConfirmButton
        ...
        onConfirm={() =>
          startTransition(async () => {
            const res = await settle(() => revokeStaff(email));
            setError(res.ok ? null : res.error);
          })
        }
      />
