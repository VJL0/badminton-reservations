...
try {
  ...
  let counts = await one(
    `select count(*) filter (where state='PLAYING')::int playing, count(*) filter (where state='QUEUED')::int queued
                            from public.session_players where session_id=$1`,
    [sessionId],
  );
  assert.deepEqual(counts, { playing: 12, queued: 8 });
  const perCourt = (
    await pool.query(
      `select count(*)::int n from public.round_players rp join public.rounds r on r.id=rp.round_id
  ...
  assert.equal(
    (
      await one(`select count(*)::int n from (select player_id from public.round_players where left_at is null
  ...
  counts = await one(
    `select count(*) filter (where state='PLAYING')::int playing, count(*) filter (where state='QUEUED')::int queued,
                             count(*) filter (where state='IDLE')::int idle from public.session_players where session_id=$1`,
    [sessionId],
  );
